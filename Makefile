...
CFLAGS += -Wa,-adhlns=$(<:%.c=$(BUILD_DIR)/$(OBJDIR)/%.lst)
...
CPPFLAGS += -Wa,-adhlns=$(<:%.cpp=$(BUILD_DIR)/$(OBJDIR)/%.lst)
...
ASFLAGS = $(ADEFS) -Wa,-adhlns=$(<:%.S=$(BUILD_DIR)/$(OBJDIR)/%.lst),-gstabs,--listing-cont-lines=100
...
OBJ = $(SRC:%.c=$(BUILD_DIR)/$(OBJDIR)/%.o) $(CPPSRC:%.cpp=$(BUILD_DIR)/$(OBJDIR)/%.o) $(ASRC:%.S=$(BUILD_DIR)/$(OBJDIR)/%.o) 

# Define all listing files.
LST = $(SRC:%.c=$(BUILD_DIR)/$(OBJDIR)/%.lst) $(CPPSRC:%.cpp=$(BUILD_DIR)/$(OBJDIR)/%.lst) $(ASRC:%.S=$(BUILD_DIR)/$(OBJDIR)/%.lst) 
...
LIBNAME=lib$(BUILD_DIR)/$(TARGET).a
lib: $(LIBNAME)
...
clean_list :
	...
	$(REMOVE) $(BUILD_DIR)/$(TARGET).cof
	$(REMOVE) $(BUILD_DIR)/$(TARGET).elf
	$(REMOVE) $(BUILD_DIR)/$(TARGET).map
	$(REMOVE) $(BUILD_DIR)/$(TARGET).sym
	$(REMOVE) $(BUILD_DIR)/$(TARGET).lss
	$(REMOVE) $(SRC:%.c=$(BUILD_DIR)/$(OBJDIR)/%.o)
	$(REMOVE) $(SRC:%.c=$(BUILD_DIR)/$(OBJDIR)/%.lst)
	$(REMOVE) $(SRC:.c=.s)
	$(REMOVE) $(SRC:.c=.d)
	$(REMOVE) $(SRC:.c=.i)
	$(REMOVEDIR) .dep
