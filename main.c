...
volatile uint16_t CURRENT_MV_VALUES[3];
volatile uint16_t NORM_MV_VALUES[3] = {1650, 1650, 1650};			// According to datasheet
...
void LcdPutFixedPoint(int16_t mili_value, uint8_t fractional_digits)
{
	 ...
	 */
	if (((abs(mili_value)/10)%10) >= 5  && fractional_digits!=2
			&& fractional_digits !=3 && mili_value<0) mili_value -= 100;
	if (((abs(mili_value)/10)%10) >= 5  && fractional_digits!=2
			&& fractional_digits !=3 && mili_value>0) mili_value += 100;
	/*
...
void LcdShowGPSPosition()
{
	...
	if (GPS_RMC_LONGITUDE[0] != '0') LcdPutChar(GPS_RMC_LONGITUDE[0]);
	else LcdPutChar(' ');
...
enum sms_type {test, alarm};
void SendSMS(uint8_t sms_type)
{
	cli();
	for (uint8_t i=0; i<10;i++)	USART1WriteChar(SMS_TEXT_MODE[i]);
	_delay_ms(200);
	for (uint8_t i=0; i<12;i++)	USART1WriteChar(SMS_SET_RECEIVER_NUMBER[i]);
	for (uint8_t i=0; i<9; i++) USART1WriteChar(IntToChar(NEW_PHONE_NUMBER[i]));
	USART1WriteChar('"');
	USART1WriteChar(13);
	_delay_ms(200);
	switch (sms_type)
	{
	case test:
		for (uint8_t i=0; i<17; i++) USART1WriteChar(SMS_TEST_TEXT[i]);
		break;

	case alarm:
		for (uint8_t i=0; i<19; i++) USART1WriteChar(SMS_TEST_ALARM[i]);
		break;
	}
	...
	USART1WriteChar('e');
	USART1WriteChar('g');
	for (uint8_t i=2; i<9; i++) USART1WriteChar(GPS_RMC_LATITUDE[i]);
	USART1WriteChar(39);
	USART1WriteChar(GPS_RMC_LATITUDE[9]);
	USART1WriteChar(' ');
	if (GPS_RMC_LONGITUDE[0] != '0') USART1WriteChar(GPS_RMC_LONGITUDE[0]);
	USART1WriteChar(GPS_RMC_LONGITUDE[1]);
	USART1WriteChar(GPS_RMC_LONGITUDE[2]);
	USART1WriteChar('d');
	USART1WriteChar('e');
	USART1WriteChar('g');
	for (uint8_t i=3; i<10; i++) USART1WriteChar(GPS_RMC_LONGITUDE[i]);
	USART1WriteChar(39);
...
prog_char txt_hello_line0[] = "MOTORBIKE";
prog_char txt_hello_line1[] = "CRASH DETECTOR";
prog_char txt_run_detector[] = "Run Detector";
prog_char txt_calibration[] = "Calibration";
prog_char txt_gps_data[] = "GPS Data";
prog_char txt_gsm_module[] = "GSM Module";
prog_char txt_set_the_origin[] = "Set the origin";
prog_char txt_run_calibration[] = "Run calibration";
prog_char txt_calibration_in_progress[] = "in progress...";
prog_char txt_alarm_triggers[] = "Alarm triggers";
prog_char txt_alarm_delay[] = "Alarm delay";
prog_char txt_gps_data_time[] = "UTC Time";
prog_char txt_gps_data_position[] = "Position";
prog_char txt_gps_data_invalid[] = "GPS disconnected";
prog_char txt_gsm_alarms_receiver[] = "Alarms receiver";
prog_char txt_gsm_send_test_sms[] = "Send test SMS";
prog_char txt_check_your_inbox[] = "Check your inbox";
...
void LcdShowMenu(uint16_t menu)
{
	switch (menu)
	{
		...
		case 241:
			LcdClear();
			if (new_alarm_delay/100 > 0) LcdPutChar(IntToChar(new_alarm_delay/100));
			else LcdGoTo(1,0);
		...
		case 311:
			LcdClear();
			LcdCursorOff();
			if (gps_rmc_valid == 1)	LcdShowGPSTime();
			else LcdPutTextP(txt_gps_data_invalid,0,0);
			break;
		case 320:
			LcdClear();
			LcdCursorOff();
		...
		case 321:
			LcdClear();
			LcdCursorOff();
			if (gps_rmc_valid == 1)	LcdShowGPSPosition();
			else LcdPutTextP(txt_gps_data_invalid,0,0);
			break;
		case 400:
			LcdClear();
...
void Key0Pressed(){
	...
	else if (menu_position == 210 || menu_position == 220 || menu_position == 230
			|| menu_position == 240) menu_position = 200;
	else if (menu_position == 211 || menu_position == 421
			|| menu_position == 311 || menu_position == 321) menu_position--;
	...
	else if (menu_position >= 232 && menu_position <= 236) menu_position --;
	...
	else if (menu_position >= 412 && menu_position <= 419) menu_position--;
	else if (menu_position == 310 || menu_position == 320) menu_position = 300;
	else if (menu_position == 410 || menu_position == 420) menu_position = 400;
...
void Key1Pressed(){
	switch (menu_position)
	{
		...
		case 230:
			...
			break;
		case 231:
			if (NEW_X_MG_TRIGGER_VALUES[0] < 3000) NEW_X_MG_TRIGGER_VALUES[0] += 10;
			break;
		case 232:
			if (NEW_X_MG_TRIGGER_VALUES[1] < 3000) NEW_X_MG_TRIGGER_VALUES[1] += 10;
			break;
		case 233:
			if (NEW_Y_MG_TRIGGER_VALUES[0] < 3000) NEW_Y_MG_TRIGGER_VALUES[0] += 10;
			break;
		case 234:
			if (NEW_Y_MG_TRIGGER_VALUES[1] < 3000) NEW_Y_MG_TRIGGER_VALUES[1] += 10;
			break;
		case 235:
			if (NEW_Z_MG_TRIGGER_VALUES[0] < 3000) NEW_Z_MG_TRIGGER_VALUES[0] += 10;
			break;
		case 236:
			if (NEW_Z_MG_TRIGGER_VALUES[1] < 3000) NEW_Z_MG_TRIGGER_VALUES[1] += 10;
			break;
		case 240:
			menu_position = 230;
			break;
		case 241:
			if (new_alarm_delay < 240) new_alarm_delay += 5;
			break;
		case 300:
			menu_position = 200;
			break;
...
void Key2Pressed(){
	switch (menu_position)
	{
		...
		case 236:
			...
			break;
		case 241:
			if (new_alarm_delay >= 5) new_alarm_delay -= 5;
			break;
		case 300:
			menu_position = 400;
			break;
		...
		case 410:
			...
			break;
		case 411:
			if (NEW_PHONE_NUMBER[0] == 0) NEW_PHONE_NUMBER[0] = 9;
			else NEW_PHONE_NUMBER[0]--;
			break;
		case 412:
			if (NEW_PHONE_NUMBER[1] == 0) NEW_PHONE_NUMBER[1] = 9;
			else NEW_PHONE_NUMBER[1]--;
			break;
		case 413:
			if (NEW_PHONE_NUMBER[2] == 0) NEW_PHONE_NUMBER[2] = 9;
			else NEW_PHONE_NUMBER[2]--;
			break;
		case 414:
			if (NEW_PHONE_NUMBER[3] == 0) NEW_PHONE_NUMBER[3] = 9;
			else NEW_PHONE_NUMBER[3]--;
			break;
		case 415:
			if (NEW_PHONE_NUMBER[4] == 0) NEW_PHONE_NUMBER[4] = 9;
			else NEW_PHONE_NUMBER[4]--;
			break;
		case 416:
			if (NEW_PHONE_NUMBER[5] == 0) NEW_PHONE_NUMBER[5] = 9;
			else NEW_PHONE_NUMBER[5]--;
			break;
		case 417:
			if (NEW_PHONE_NUMBER[6] == 0) NEW_PHONE_NUMBER[6] = 9;
			else NEW_PHONE_NUMBER[6]--;
			break;
		case 418:
			if (NEW_PHONE_NUMBER[7] == 0) NEW_PHONE_NUMBER[7] = 9;
			else NEW_PHONE_NUMBER[7]--;
			break;
		case 419:
			if (NEW_PHONE_NUMBER[8] == 0) NEW_PHONE_NUMBER[8] = 9;
			else NEW_PHONE_NUMBER[8]--;
			break;
	}
}
void Key2Released(){
...
void Key3Pressed(){
	if (menu_position == 100 || menu_position == 200 || menu_position == 300
	   || menu_position == 400) menu_position += 10;
			...
			|| (menu_position >= 230 && menu_position <= 235) || menu_position == 240
			|| menu_position == 310 || menu_position == 320
			|| (menu_position >= 410 && menu_position <= 418)) menu_position++;
	else if (menu_position == 211) SetOrigin();
...
int main()
{
	...
	while(1)
	{
		...
		if (gps_overwrite_allowed == 0)
		{
			...
			{
				gps_rmc_valid = 1;
				for (uint8_t i=7; i<13; i++) GPS_RMC_TIME[i-7] = GPS_RECEIVED_DATA[i];
				for (uint8_t i=20; i<29; i++) GPS_RMC_LATITUDE[i-20] = GPS_RECEIVED_DATA[i];
				GPS_RMC_LATITUDE[9] = GPS_RECEIVED_DATA[30];
				for (uint8_t i=32; i<42; i++) GPS_RMC_LONGITUDE[i-32] = GPS_RECEIVED_DATA[i];
				GPS_RMC_LONGITUDE[10] = GPS_RECEIVED_DATA[43];
				if (GPS_RECEIVED_DATA[55] == ',')
				{
					for (uint8_t i=56; i<62; i++) GPS_RMC_DATE[i-56] = GPS_RECEIVED_DATA[i];
				}
				else if (GPS_RECEIVED_DATA[56] == ',')
				{
					for (uint8_t i=57; i<63; i++) GPS_RMC_DATE[i-57] = GPS_RECEIVED_DATA[i];
				}
			}
			else gps_rmc_valid = 0;
		...
		if (menu_position == 110)
		{
			if   (((CURRENT_MG_VALUES[0] < NEW_X_MG_TRIGGER_VALUES[0]) && (CURRENT_MG_VALUES[0] > -3000))
			   || ((CURRENT_MG_VALUES[0] > NEW_X_MG_TRIGGER_VALUES[1]) && (CURRENT_MG_VALUES[0] <  3000))
			   || ((CURRENT_MG_VALUES[1] < NEW_Y_MG_TRIGGER_VALUES[0]) && (CURRENT_MG_VALUES[1] > -3000))
			   || ((CURRENT_MG_VALUES[1] > NEW_Y_MG_TRIGGER_VALUES[1]) && (CURRENT_MG_VALUES[1] <  3000))
			   || ((CURRENT_MG_VALUES[2] < NEW_Z_MG_TRIGGER_VALUES[0]) && (CURRENT_MG_VALUES[2] > -3000))
			   || ((CURRENT_MG_VALUES[2] > NEW_Z_MG_TRIGGER_VALUES[1]) && (CURRENT_MG_VALUES[2] <  3000))) alarm_1st_grade = 1;
...
ISR(TIMER2_OVF_vect)
{
	...
	if (timer2_ovf_counter%10 == 3)
	{
		...
		if (alarm_1st_grade == 1) PORTD ^= _BV(6);
		else if (alarm_2nd_grade == 1) PORTD |= _BV(6);
		else PORTD &= ~_BV(6);
	}
	if (timer2_ovf_counter == 0)
	{
		if ((alarm_1st_grade == 1) && (new_alarm_delay != 0)) new_alarm_delay--;
	}
}
...
ISR(USART0_RX_vect)
{
	...
	{
		if (usart0_received_data == '$') gps_array_pointer = 0;
		else gps_array_pointer++;
		GPS_RECEIVED_DATA[gps_array_pointer] = usart0_received_data;
		if (usart0_received_data == '*' && GPS_RECEIVED_DATA[3] == 'R' && GPS_RECEIVED_DATA[4] == 'M'
			&& GPS_RECEIVED_DATA[5] == 'C') gps_overwrite_allowed = 0;
	}
}
